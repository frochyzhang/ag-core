...
type use interface {
	Use(mws ...hertzclient.Middleware)
}
...
type Options struct {
	...
	middlewares           []hertzclient.Middleware
...
func WithHertzClientMiddleware(mws ...hertzclient.Middleware) Option {
	return Option{func(op *Options) {
...
func withNamingClient(namingClient naming_client.INamingClient) Option {
	return Option{func(op *Options) {
		r := nacos.NewNacosResolver(namingClient)
		op.middlewares = append(op.middlewares, sd.Discovery(r))
	}}
}
...
func (c *cli) Use(mws ...hertzclient.Middleware) error {
	u, ok := c.doer.(use)
	if !ok {
...
func newClient(opts *Options) (*cli, error) {
	...
	if opts.doer == nil {
		cli, err := hertzclient.NewClient(opts.clientOption...)
		if err != nil {
			return nil, err
		}
		opts.doer = cli
