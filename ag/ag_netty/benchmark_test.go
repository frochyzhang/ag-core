...
func benchEchoServer(b *testing.B) (*Server, string) {
	b.Helper()
...
func benchClient(b *testing.B, addr string) *Client {
	b.Helper()
...
func BenchmarkEchoRoundTrip(b *testing.B) {
	srv, addr := benchEchoServer(b)
	defer srv.Shutdown()

	c := benchClient(b, addr)
	defer c.Close()
...
func BenchmarkEchoPayloadSize(b *testing.B) {
...
func BenchmarkSendFireAndForget(b *testing.B) {
	srv, addr := benchEchoServer(b)
	defer srv.Shutdown()
...
func BenchmarkEventLoopPostParallel(b *testing.B) {
	el := NewEventLoop()
	defer el.Shutdown()
...
func BenchmarkFutureCreateComplete(b *testing.B) {
	b.ReportAllocs()
...
func BenchmarkFutureGetWithTimeout(b *testing.B) {
	b.ReportAllocs()

	for i := 0; i < b.N; i++ {
		f := NewFuture()
		go func() { f.Complete("result") }()
		_, _ = f.GetWithTimeout(5 * time.Second)
	}
}

func BenchmarkPipelineFireRead(b *testing.B) {
...
func BenchmarkPipelineMultiHandler(b *testing.B) {
...
func (h *countingHandler) HandleActive(ctx *HandlerContext)           {}
func (h *countingHandler) HandleInactive(ctx *HandlerContext)         {}
func (h *countingHandler) HandleError(ctx *HandlerContext, err error) {}
func (h *countingHandler) HandleWrite(ctx *HandlerContext, data []byte) {
	ctx.Channel().WriteDirect(data)
}
func (h *countingHandler) HandleRead(ctx *HandlerContext, data []byte) {
	h.count.Add(1)
}
...
func (h *passThroughHandler) HandleActive(ctx *HandlerContext)           {}
func (h *passThroughHandler) HandleInactive(ctx *HandlerContext)         {}
func (h *passThroughHandler) HandleError(ctx *HandlerContext, err error) {}
func (h *passThroughHandler) HandleWrite(ctx *HandlerContext, data []byte) {
	ctx.Channel().WriteDirect(data)
}
func (h *passThroughHandler) HandleRead(ctx *HandlerContext, data []byte) {
	h.count.Add(1)
}
